...
object KernelGenerator {
  def apply(): KernelGenerator =
    new KernelGenerator(
      OpenCL.CodeGeneration.CodeGenerator(),
      OpenCL.AST.Printer(_))
}
//noinspection VariablePatternShadow
class KernelGenerator(
  val codeGen: C.CodeGeneration.CodeGenerator,
  ...
  def makeCode[T <: PhraseType](
    ...
    makeKernel(name, phrase, params.reverse, defs.reverse,
      Some(localSize), Some(globalSize)).right.get
  }
  ...
  private def makeKernel(
    ...
    adaptKernelParameters(
      ...
      type IdentMap = Predef.Map[Identifier[_ <: BasePhraseTypes], DeclRef]
    ...
    SimplifyNats(
      UnrollLoops(
        FlagPrivateArrayLoops(
          InjectWorkItemSizes(localSize, globalSize)(
            TranslationToImperative.acc(
              p)(a)(new OpenCL.TranslationContext())
              |> (p => {
